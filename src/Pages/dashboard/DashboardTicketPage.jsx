...
function WelcomeHeader() {
  return (
    <header>
      <p className="text-sm text-slate-500">Welcome back! Here's your ticket overview</p>
    </header>
  );
}
      ...
      {cards.map((card, index) => (
        <div key={index} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
          <div className="flex items-start justify-between mb-4">
            <div className={`${colorClasses[card.color]} rounded-xl p-3 text-white`}>
              {card.icon}
            </div>
            <div className={`flex items-center text-xs font-medium ${card.isPositive ? 'text-green-600' : 'text-red-600'}`}>
              <svg className="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={card.isPositive ? "M5 10l7-7m0 0l7 7m-7-7v18" : "M19 14l-7 7m0 0l-7-7m7 7V3"} />
              </svg>
              {card.change}
            </div>
          </div>
...
function TicketsPerWeekChart() {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <h2 className="text-lg font-semibold text-slate-800 mb-6">Tickets per Week</h2>
...
function TicketTrendChart() {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      <h2 className="text-lg font-semibold text-slate-800 mb-6">Ticket Trend</h2>
      <div className="relative">
        <svg width="100%" height="240" viewBox="0 0 600 240" className="overflow-visible">
          {/* Grid lines */}
          <line x1="40" y1="30" x2="560" y2="30" stroke="#e2e8f0" strokeWidth="1" strokeDasharray="5,5" />
          <line x1="40" y1="105" x2="560" y2="105" stroke="#e2e8f0" strokeWidth="1" strokeDasharray="5,5" />
          <line x1="40" y1="180" x2="560" y2="180" stroke="#e2e8f0" strokeWidth="1" strokeDasharray="5,5" />

          {/* Y-axis labels */}
          <text x="20" y="35" fontSize="12" fill="#64748b" textAnchor="end">80</text>
          <text x="20" y="110" fontSize="12" fill="#64748b" textAnchor="end">60</text>
          <text x="20" y="185" fontSize="12" fill="#64748b" textAnchor="end">40</text>
          <text x="20" y="220" fontSize="12" fill="#64748b" textAnchor="end">0</text>
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-200">
        ...
        <div className="text-sm text-slate-600">
          Avg: <span className="font-semibold text-slate-900">{Math.round(trendData.reduce((sum, d) => sum + d.count, 0) / trendData.length)}</span>
        </div>
      </div>
    </div>
  );
